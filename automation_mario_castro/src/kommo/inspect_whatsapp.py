...
CONVERSATION_PATHS = ["/leads/list/", "/todo/", "/imbox/", "/settings/communications/"]
...
def main() -> None:
    ...
    with browser_context("kommo") as (_, _, _, page):
        ...
        wait_for_human_if_needed(page, "Kommo WhatsApp")
        ...
        for path in CONVERSATION_PATHS:
            ...
            try:
                ...
                save_page_screenshot(page, screenshot_path(f"kommo_whatsapp_{path.strip('/').replace('/', '_') or 'root'}.png"))
            except Exception as exc:
                evidence.append(f"\n\n## URL intentada: {candidate}\n\nERROR: {exc}")

        inspect_path("kommo_whatsapp.txt").write_text("\n".join(evidence), encoding="utf-8")
        print("[inspect] evidencia WhatsApp guardada")
