...
_SYSTEM_PROMPT_TEMPLATES: dict[str, str] = {
    "es": (
        ...
        "- SEPARACIÓN DE ETAPAS: El diagnóstico público es solo de factibilidad.\n"
        "  - NO preguntes por pestañas exactas, nombres de hoja, cuentas de Google, "
        "permisos editor, OAuth, credenciales, API, webhooks, service accounts, "
        "columnas exactas de planillas ni configuración técnica.\n"
        "  - Esos puntos corresponden a la etapa posterior de ANÁLISIS DE FLUJO.\n"
        "  - Si el usuario los menciona, indicá que se validan en la etapa de "
        "análisis de flujo para implementación.\n"
        "- CIERRE DE FACTIBILIDAD: Cuando tengas canal + sistema actual + objetivo "
        "+ al menos un dato extra (volumen, aprobación humana, criterio operativo), "
        "cerrá el diagnóstico. No sigas preguntando.\n\n"
...
class PromptPolicy:
    ...
    def build_turn_prompt(
        ...
        if is_diagnose and sd:
            ...
            parts.append(
                "\nInstrucciones para esta respuesta (ACCIÓN: DIAGNÓSTICO DE FACTIBILIDAD):\n"
                "- NO hagas preguntas. El usuario ya proporcionó información suficiente.\n"
                "- Este es un DIAGNÓSTICO DE FACTIBILIDAD, no un análisis de implementación.\n"
                "- Generá el diagnóstico completo ahora.\n"
                "- Si falta un detalle de implementación (regla exacta de descuento, umbral, horario, "
                "pestaña exacta, cuenta de Google, permisos, OAuth, API, webhook, columnas de planilla), "
                "incluilo como PUNTO A VALIDAR TÉCNICAMENTE EN ETAPA POSTERIOR, no preguntes por él.\n"
                "- El diagnóstico debe incluir:\n"
                "  1. Hechos confirmados (canal, proceso, sistema actual, objetivo)\n"
                "  2. Factibilidad (alta/media/baja + breve explicación)\n"
                "  3. Modo recomendado (asistido / con aprobación humana / automático parcial)\n"
                "  4. Qué automatizar primero (clasificación, registro, derivación, trazabilidad)\n"
                "  5. Qué NO automatizar todavía (cierre automático, decisiones con riesgo)\n"
                "  6. Riesgos principales (calidad de datos, reglas ambiguas, control humano)\n"
                "  7. Puntos a validar técnicamente después (acceso, permisos, integración)\n"
                "  8. Próximo paso: análisis de flujo para implementación\n"
                "- No sigas preguntando. El diagnóstico de factibilidad está completo.\n"
                f"- {LANGUAGE_INSTRUCTIONS.get(response_lang, f'Respond only in {response_lang}.')}"
            )
        else:
            ...
            if offer_pause and not user_wants_continue:
                parts.append(
                    ...
                    "- Hacé un resumen de una línea de lo que ya sabés.\n"
                    ...
                    "- No uses la misma frase de apertura en cada turno. "
                    "Variá naturalmente.\n"
                ...
                parts.append(
                    ...
                    "- NO preguntes por pestañas exactas, nombres de hoja, cuentas de Google, "
                    "permisos, OAuth, credenciales, API, webhooks ni columnas de planillas. "
                    "Esos corresponden a la etapa de análisis de flujo posterior.\n"
                    "- Cuando tengas canal + sistema actual + objetivo + volumen o aprobación humana, "
                    "no sigas preguntando. Dejá que el runtime cierre el diagnóstico.\n"
