"""Inspection probe for Mercado Libre authenticated account summary discovery."""
...
from ..browser.actions import (
    collect_summary_inspection,
    maybe_save_storage_state,
    save_debug_screenshot,
    save_text_report,
)
from ..browser.config import DEFAULT_PROFILE_NAME, INSPECT_DIR, SUMMARY_DISCOVERY_KEYWORDS
from ..browser.context import close_context, open_persistent_context
from ..browser.pages import open_account_summary, stabilize_summary
from ..browser.session_store import get_screenshot_path, get_storage_state_path


def parse_args() -> argparse.Namespace:
    """Parse CLI arguments."""
    parser = argparse.ArgumentParser(description="Account summary inspection smoke for Mercado Libre persistent browser sessions.")
    parser.add_argument("--profile", default=DEFAULT_PROFILE_NAME, help="Persistent browser profile name.")
    parser.add_argument("--timeout", type=int, default=90, help="Seconds reserved for future summary loading adjustments.")
...
def _get_report_path(profile_name: str) -> Path:
    """Return a timestamped text report path for summary inspection."""
    stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    return INSPECT_DIR / f"summary_inspect_{_safe_name(profile_name)}_{stamp}.txt"
...
def _format_report(profile_name: str, inspection: dict[str, object]) -> str:
    """Render a compact text report for manual summary discovery."""
...
def run(profile_name: str, timeout_sec: int) -> int:
    """Execute the authenticated summary inspection flow."""
    screenshot_path = get_screenshot_path(prefix=f"smoke_summary_inspect_{profile_name}")
    ...
    with sync_playwright() as playwright:
        try:
            ...
            log("abriendo summary de cuenta Mercado Libre")
            open_account_summary(page)
            settle_ms = max(3_000, min(max(timeout_sec, 1), 10) * 500)
            stabilize_summary(page, settle_ms=settle_ms)

            inspection = collect_summary_inspection(page, keywords=SUMMARY_DISCOVERY_KEYWORDS)
            ...
            log(f"error durante smoke summary inspect: {exc}")
        finally:
            try:
