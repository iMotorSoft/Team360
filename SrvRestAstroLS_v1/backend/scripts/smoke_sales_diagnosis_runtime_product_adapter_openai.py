"""Smoke HTTP para el product adapter con OpenAI direct opt-in.
...
La API key se resuelve desde globalVar.py via get_team360_openai_key()
con prioridad: OpenAI_Key_JAI_query > TEAM360_OPENAI_KEY > OPENAI_API_KEY.
El modelo se resuelve via get_team360_openai_model() (default gpt-5-nano).

Si TEAM360_SALES_DIAGNOSIS_PRODUCT_LLM_PROVIDER=openai no esta seteado
o la key no es resoluble desde globalVar.py / env, el script hace SKIP
controlado (exit 0, no es fallo).

Flags:
    --allow-fallback   No fallar si OpenAI devolvio SAFE_ACK_TEXT
                       (fallback seguro). Por defecto el smoke falla
                       si se detecta que OpenAI no respondio realmente.

Uso:
    # Terminal 1 — backend con product adapter habilitado + OpenAI:
    TEAM360_SALES_DIAGNOSIS_PRODUCT_ROUTE_ENABLED=1 \
    TEAM360_SALES_DIAGNOSIS_PRODUCT_STATE_REPOSITORY=inmemory_test \
    TEAM360_SALES_DIAGNOSIS_PRODUCT_LLM_PROVIDER=openai \
      uv run uvicorn app:app --host 127.0.0.1 --port 8018

    # Terminal 2 — smoke OpenAI product adapter:
    TEAM360_SALES_DIAGNOSIS_PRODUCT_LLM_PROVIDER=openai \
      uv run python scripts/smoke_sales_diagnosis_runtime_product_adapter_openai.py

    # Con allow-fallback (no falla si OpenAI falla):
    TEAM360_SALES_DIAGNOSIS_PRODUCT_LLM_PROVIDER=openai \
...
exportarlos explicitamente en el shell si ya estan configurados en
.bashrc o entorno via OpenAI_Key_JAI_query, TEAM360_OPENAI_KEY
o TEAM360_OPENAI_MODEL.
"""
...
UNIQUE_SESSION = f"smoke_product_openai_{uuid4().hex[:12]}"
CHECKS: list[str] = []
FAILURES: list[str] = []
...
def _detect_openai_envs() -> tuple[str, str, str, bool]:
    ...
    try:
        ...
        pass
    if not api_key:
        api_key = os.environ.get("TEAM360_OPENAI_KEY", "").strip() or os.environ.get("OPENAI_API_KEY", "").strip()
    return route, state, llm, bool(api_key)
...
def run_smoke(base_url: str, timeout: float, allow_fallback: bool = False) -> None:
    ...
    route_env, state_env, llm_env, has_api_key = _detect_openai_envs()
    print("=== Sales Diagnosis Product Adapter — OpenAI Smoke ===")
    ...
    print(f"OpenAI key (globalVar):    {'yes' if has_api_key else 'no'}")
    print(f"Session ID:                {UNIQUE_SESSION}")
    print(f"Allow fallback:            {allow_fallback}")
    print()

    # ------------------------------------------------------------------
    # Gate: require openai llm provider + api key
    # ------------------------------------------------------------------
    if llm_env != "openai":
        print("--- SKIP ---")
        print(f"  {PRODUCT_LLM_PROVIDER_ENV}={llm_env!r}")
        print("  Set to 'openai' and restart the backend to run this smoke.")
        print()
    ...
    if not has_api_key:
        ...
        print("  globalVar.get_team360_openai_key() returned empty.")
        print("  Configure via OpenAI_Key_JAI_query, TEAM360_OPENAI_KEY,")
        print("  or OPENAI_API_KEY in env or globalVar.py")
        print("  to run this smoke.")
        print()
        print("Result: SKIPPED (not a failure)")
        sys.exit(0)

    # ------------------------------------------------------------------
    # 1. Valid request returns 201 with response
    # ------------------------------------------------------------------
    ...
    if status != 201 or not isinstance(body, dict):
        # Cannot continue without valid response
    ...
    # ------------------------------------------------------------------
    # 7. No LiteLLM real by default
    # ------------------------------------------------------------------
    print("--- 7. No real LiteLLM ---")
    _check(
    ...
    # ------------------------------------------------------------------
    # 8. No real DB info leaked
    # ------------------------------------------------------------------
    print("--- 8. No real DB leaked ---")
    _check(
    ...
    # ------------------------------------------------------------------
    # 9. Provider result event: distinguish real vs fallback
    # ------------------------------------------------------------------
    print("--- 9. Provider result (real OpenAI vs fallback) ---")
    ...
    if provider_event is None:
        ...
        if is_fallback:
            _check(
                "OpenAI responded (not fallback)",
                False,
                "Response is SAFE_ACK_TEXT fallback — OpenAI may not be "
                "configured correctly or API call failed",
            )
            if allow_fallback:
                print("  (--allow-fallback set, treating fallback as acceptable)")
                # Override the check result for summary
    ...
    # ------------------------------------------------------------------
    # 10. turn_count increments in same session
    # ------------------------------------------------------------------
    print("--- 10. turn_count increments ---")
    _, body2 = _post_turn(
        base_url,
        UNIQUE_SESSION,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Smoke for the product adapter sales diagnosis endpoint with OpenAI direct opt-in"
    )
    parser.add_argument(
        "--backend-url",
    ...
    parser.add_argument(
        "--allow-fallback",
        action="store_true",
        help="Do not fail if OpenAI returned SAFE_ACK_TEXT fallback instead of real response",
    )
