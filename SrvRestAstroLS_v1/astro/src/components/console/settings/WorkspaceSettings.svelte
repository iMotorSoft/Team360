<script lang="ts">
  ...
  const workspaceIntegrations = $derived(integrations.filter(({ workspaceId }) => workspaceId === consoleContext.activeWorkspace.id));
</script>
  ...
  <div class="mt-7 grid gap-5 xl:grid-cols-2">
    <Card variant="flat-large">
      ...
      <dl class="mt-5 space-y-3 text-sm">
        ...
        <div class="flex justify-between gap-3 border-b border-[#edf1f2] pb-3"><dt class="text-[#78909f]">Workspace</dt><dd class="font-bold text-[#31536b]">{consoleContext.activeWorkspace.name}</dd></div>
        <div class="flex justify-between gap-3 border-b border-[#edf1f2] pb-3"><dt class="text-[#78909f]">Región</dt><dd class="font-bold text-[#31536b]">{consoleContext.activeOrganization.region}</dd></div>
    ...
    <Card variant="flat-large">
      ...
      <div class="mt-4 flex flex-wrap gap-2">
        {#each consoleContext.bootstrap.enabledModules as module}
          <span class="rounded-full bg-[#eef6f5] px-2.5 py-1 text-[0.66rem] font-bold text-[#47716f]">{module}</span>
        {/each}
      </div>
      ...
      <div class="mt-4 space-y-3">
        {#each consoleContext.contractedServices as service}
          <div class="flex items-center justify-between gap-3 rounded-xl bg-[#f8fbfa] px-3 py-3">
            <div>
              <p class="text-xs font-bold text-[#47657b]">{service.name}</p>
              <p class="mt-1 text-[0.67rem] text-[#91a2ad]">{service.packageName}</p>
            </div>
      ...
      <div class="mt-4 space-y-3">
        {#each workspaceIntegrations as integration}
          <div class="rounded-xl bg-[#f8fbfa] px-3 py-3">
            ...
            <p class="mt-2 text-[0.67rem] leading-5 text-[#91a2ad]">{integration.note}</p>
          </div>
        {:else}
