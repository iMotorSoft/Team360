<script lang="ts">
  ...
  function workspaceNames(workspaceIds: string[]) {
    return workspaces
      .filter(
        ({ id }) => workspaceIds.includes(id) && accessibleWorkspaceIds.has(id),
      )
      .map(({ name }) => name);
  }
</script>
...
<section>
  <SectionHeader
    ...
    title={consoleContext.bootstrap.uiHints.technicalDepth === "full"
      ? "Usuarios"
      : "Equipo"}
    ...
    {#each visibleUsers as user}
      <Card variant="flat" class="">
        <div class="flex items-start gap-4">
          <span
            class="grid size-12 shrink-0 place-items-center rounded-full bg-[#153b5b]
          text-base font-bold text-white tracking-[0.07em]"
          ...
          <div class="min-w-0 flex-1">
            ...
            >
              <div>
                <h2 class="text-lg font-bold text-[#31536b]">{user.name}</h2>
                <p class="mt-1 text-lg text-[#8396a2]">{user.email}</p>
              </div>
              <StatusBadge status={user.status} />
            </div>
            <p class="mt-3 top-badge">
              {user.role}
            </p>
        ...
        <div class="flex flex-col mt-4 border-t border-[#edf1f2] pt-4">
          <p class="px-2.5 top-badge-neutral">Workspaces visibles</p>
          <p class="mt-2 px-2.5 text-base leading-5 text-[#668092]">
            {workspaceNames(user.workspaceIds).join(" · ") ||
              "Sin workspace visible en este alcance"}
          </p>
          <div class="grow"></div>
          <div class="mt-6 px-1 flex flex-wrap gap-2">
            {#each user.permissionSummary as permission}
              <span
                class="rounded-full bg-[#eef6f5] px-2.5 py-1 text-base
                font-bold text-[#47716f]">{permission}</span
              >
            {/each}
          </div>
        </div>
