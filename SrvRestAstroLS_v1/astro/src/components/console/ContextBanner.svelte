...
<section
  class={`flex flex-col gap-3 rounded-2xl border px-4 py-3 sm:flex-row sm:items-center sm:justify-between ${
    consoleContext.bootstrap.uiHints.showDelegatedAccessNotice
      ? "border-[#f0d8a5] bg-[#fff9ec]"
      : "border-[#dce8e8] bg-white/76"
  }`}
>
  <div>
    <div class="flex flex-wrap items-center gap-2">
      ...
      {:else}
        <Badge variant="info" class="h-auto px-2 py-1 text-sm"
          >Contexto propio</Badge
        >
      {/if}
    </div>
    <p class="mt-1.5 text-base font-semibold text-[#254b66]">
      {consoleContext.activeOrganization.name}
      <span class="mx-1.5 text-[#a1b0b8]">·</span>
      {consoleContext.activeWorkspace.name}
    </p>
  </div>
  <p class="max-w-xl text-sm leading-5 text-[#718793]">
    {#if consoleContext.bootstrap.uiHints.showDelegatedAccessNotice}
      Estás consultando una organización autorizada desde {audience ===
        "owner" || audience === "operator"
        ? "Team360"
        : "tu red partner"}.
    {:else}
      La información visible corresponde al workspace seleccionado y a sus
