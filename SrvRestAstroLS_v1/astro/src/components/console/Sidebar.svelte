<script lang="ts">
  import { APP_PUBLIC_NAME } from "../global.js";
  ...
  import team360Logo from "../../assets/team360_logo_t.png";
...
<aside
  ...
  class={`console-drawer fixed inset-y-0 start-0 z-50 flex w-[20rem] flex-col border-e border-[#dce6e8] bg-[#f8fbfa] transition-transform duration-300 lg:translate-x-0 ${
    mobileOpen ? "translate-x-0" : consoleContext.direction === "rtl" ? "translate-x-full" : "-translate-x-full"
  }`}
>
  <div class="flex h-24 items-center justify-between border-b border-[#e2eaec] px-5">
    <a class="flex items-center gap-3" href="/" translate="no">
      ...
      ></span>
      <span>
        <span class="block text-2xl font-bold tracking-[-0.05em] text-[#233B56]">{APP_PUBLIC_NAME.split(" ")[0]}</span>
        <span class="block text-xs font-bold uppercase tracking-[0.2em] text-[#168b88]">{APP_PUBLIC_NAME.split(" ")[1]}</span>
      </span>
    </a>
    <button
      aria-label="Cerrar navegación"
      ...
      <div>
        ...
        <ul class="mt-2 space-y-1">
          {#each group.items as item}
            <li>
              <a
                class={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-lg font-semibold transition focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#168b88] ${
                  item.view === view ? "bg-[#e4f5f3] text-[#147d79]" : "text-[#587184] hover:bg-base-200 hover:text-[#214762]"
                }`}
                href={item.href}
                onclick={onClose}
              >
  ...
  <div class="border-t border-[#e2eaec] p-4">
    <div class="rounded-2xl border border-[#d8e7e7] bg-white p-3">
      ...
      <p class="mt-1.5 text-base leading-5 text-[#6d8391]">Datos simulados. Sin autenticación ni operaciones reales.</p>
    </div>
  </div>
</aside>
