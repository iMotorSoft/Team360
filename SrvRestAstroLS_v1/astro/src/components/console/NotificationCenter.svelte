...
<div class="relative">
  <button
    ...
    class="relative grid size-11 cursor-pointer place-items-center rounded-full text-[#526d81] transition hover:bg-slate-100"
    onclick={() => (open = !open)}
    type="button"
  >
  ...
  {#if open}
    <div class="absolute end-0 z-50 mt-3 w-[min(24rem,calc(100vw-2rem))] overflow-hidden rounded-2xl border border-[#dfe8ea] bg-white shadow-[0_24px_70px_-28px_rgba(16,45,79,0.35)]">
        ...
        {#each alerts as alert}
          <article class="rounded-xl px-3 py-3 transition hover:bg-[#f4f8f8]">
            <div class="flex items-center justify-between gap-3">
              ...
              <span class="text-sm text-[#8a9ba6]">{formatDateTime(alert.createdAt, consoleContext.locale)}</span>
            </div>
            <p class="mt-2 text-sm font-semibold leading-5 text-[#36566f]">{alert.title}</p>
          </article>
        {:else}
          <p class="px-3 py-6 text-center text-lg text-[#78909f]">Sin alertas abiertas en este workspace.</p>
        {/each}
      </div>
    </div>
  {/if}
