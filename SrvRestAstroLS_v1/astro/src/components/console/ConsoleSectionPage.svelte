<script lang="ts">
  ...
  const accessibleOrganizationIds = $derived(
    new Set(
      consoleContext.bootstrap.accessibleOrganizations.map(({ id }) => id),
    ),
  );
  ...
  const visibleTasks = $derived(
    tasks.filter(({ workspaceId }) => accessibleWorkspaceIds.has(workspaceId)),
  );
  ...
  const clients = $derived(
    consoleContext.bootstrap.accessibleOrganizations.filter(({ type }) =>
      ["direct_client", "partner_client"].includes(type),
    ),
  );
  ...
  const copy = $derived.by(() => {
    const labels: Record<ConsoleView, [string, string, string]> = {
      ...
      organizations: [
        "Organizaciones",
        "Estructura autorizada de la red",
        "Consulta organizaciones, jerarquía, región y estado operativo.",
      ],
      partners: [
        "Partners",
        "Canales regionales configurables",
        "Seguimiento de distribuidores y activaciones dentro de la plataforma.",
      ],
      clients: [
        audience === "partner" ? "Mis clientes" : "Clientes",
        "Organizaciones cliente visibles",
        "Accede a cada contexto permitido sin mezclar datos entre organizaciones.",
      ],
      workspaces: [
        "Workspaces",
        "Contextos operativos",
        "Cada workspace delimita servicios, configuraciones y resultados visibles.",
      ],
      services: [
        audience === "client" ? "Servicios contratados" : "Servicios",
        "Prestaciones habilitadas",
        "Revisa estado, salud, paquete y próximo paso de cada servicio.",
      ],
      "client-services": [
        "Servicios de clientes",
        "Prestaciones dentro de tu red",
        "Consulta los servicios activos o en onboarding de clientes gestionados.",
      ],
      results: [
        "Resultados",
        "Indicadores comprensibles",
        "Métricas visibles del workspace activo para orientar decisiones y próximos pasos.",
      ],
      workers: [
        "Workers",
        "Capacidades técnicas autorizadas",
        "Vista interna de componentes operativos y ejecuciones relacionadas.",
      ],
      runs: [
        "Ejecuciones",
        "Actividad técnica reciente",
        "Seguimiento interno de runs, estados y resúmenes seguros.",
      ],
      reports: [
        "Reportes",
        "Salidas consultables",
        "Reportes generados y programados para el contexto operativo seleccionado.",
      ],
      alerts: [
        "Alertas",
        "Situaciones que requieren atención",
        "Prioriza eventos visibles, severidad y estado dentro del workspace.",
      ],
      tasks: [
        "Tareas",
        "Próximas acciones",
        "Trabajo manual, asistido o pendiente de aprobación humana.",
      ],
      team: [
        audience === "owner" ? "Usuarios" : "Equipo",
        "Personas y accesos visibles",
        "Usuarios autorizados dentro del alcance seleccionado.",
      ],
      support: [
        "Soporte",
        "Seguimiento contextualizado",
        "Centraliza consultas y escalamiento con el contexto operativo visible.",
      ],
      settings: [
        "Configuración",
        "Preferencias del contexto",
        "Parámetros visibles de experiencia, idioma y alcance mock.",
      ],
    };
  ...
  function statusVariant(
    ...
    if (["active", "healthy", "completed", "ready"].includes(status))
      return "success";
    if (
      [
        "attention",
        ...
        <Card variant="flat" padding="p-4">
          ...
          <p class="mt-4 border-t border-[#edf1f2] pt-3 text-xs text-[#78909f]">
            Workspace: {workspaces.find(
              ({ organizationId }) => organizationId === client.id,
            )?.name ?? "Activación pendiente"}
          </p>
        </Card>
      {/each}
    </div>
        ...
        <table class="w-full min-w-[42rem] text-start text-sm">
          ...
          >
            <tr
              ...
              ><th class="px-5 py-3 text-start">Región</th><th
                class="px-5 py-3 text-start">Workspace</th
              ><th class="px-5 py-3 text-start">Estado</th></tr
            >
          </thead>
    ...
    <div class="mt-7 grid gap-4 xl:grid-cols-2">
      ...
      {:else}
        <Card tag="p" variant="flat" class="text-sm text-[#718793]"
          >No hay servicios visibles para este contexto.</Card
        >
      {/each}
    </div>
  {:else if view === "results"}
    <div class="mt-7 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {#each context.services as service}
        {#each service.metrics as metric}
          <Card variant="flat">
            <p class="top-badge-neutral">
              {service.name}
            </p>
            <p
              class="mt-4 text-3xl font-bold tracking-[-0.06em] text-[#173b5b]"
            >
              {metric.value}
            </p>
            <p class="details-text mt-2 font-semibold">
              {metric.label}
            </p>
            {#if metric.trend}<p
                class="mt-2 text-base font-bold text-[#168b88]"
              >
                {metric.trend}
              </p>{/if}
          </Card>
    ...
    <div class="mt-7 grid gap-3 lg:grid-cols-2">
      {#each Array.from(new Set(visibleRuns.map(({ workerName }) => workerName))) as workerName}
        <Card variant="flat">
          ...
          <p class="mt-2 text-xs leading-5 text-[#78909f]">
            Capacidad técnica interna · logs resumidos · sin secretos expuestos.
          </p>
          <div
            class="mt-4 flex items-center justify-between border-t border-[#edf1f2] pt-3 text-xs text-[#78909f]"
          >
            <span
              >{visibleRuns.filter((run) => run.workerName === workerName)
                .length} run(s)</span
            >
            <Badge variant="success" class="h-auto px-2 py-1 text-[0.62rem]"
              >visible</Badge
            >
          </div>
        </Card>
        ...
        <Card variant="flat">
          ...
          >
            <div>
              ...
              <p class="mt-1 text-xs text-[#8396a2]">
                {serviceForId(run.serviceId)?.name}
              </p>
            </div>
            <Badge
          ...
          <p class="mt-3 text-[0.68rem] text-[#91a2ad]">
            {formatDateTime(run.startedAt, consoleContext.locale)}
            {run.finishedAt
              ? `→ ${formatDateTime(run.finishedAt, consoleContext.locale)}`
              : "· en curso"}
          </p>
        </Card>
      {/each}
  ...
  {:else if view === "alerts"}
    <div class="mt-7 space-y-3">
      ...
      {:else}
        ...
        >
          No hay alertas registradas para el contexto activo.
        </p>
      {/each}
    </div>
  {:else if view === "tasks"}
    <div class="mt-7 space-y-3">
      {#each context.tasks as task}
        ...
        >
          <div>
            ...
            <p class="mt-1 text-xs text-[#8396a2]">
              {serviceForId(task.serviceId)?.name} · vence {formatDate(
                task.dueDate,
      ...
      {:else}
        <p
          class="rounded-2xl border border-[#dce8e8] bg-white p-5 text-sm text-[#718793]"
        >
          No hay tareas pendientes para este workspace.
        </p>
      {/each}
    </div>
  {:else if view === "team"}
    <div class="mt-7 grid gap-3 lg:grid-cols-2">
      {#each visibleUsers as user}
        <Card variant="flat" padding="p-4" class="flex items-center gap-4">
          <span
            class="grid size-11 shrink-0 place-items-center rounded-xl bg-[#153b5b] text-xs font-bold text-white"
          ...
          <div class="min-w-0">
            ...
            <p
              class="mt-1 text-[0.68rem] font-bold uppercase tracking-[0.1em] text-[#168b88]"
            >
              {user.role}
            </p>
          </div>
    ...
    <div class="mt-7 grid gap-4 lg:grid-cols-2">
      <Card variant="flat-large">
        ...
        <dl class="mt-4 space-y-3 text-sm">
          <div
            class="flex justify-between gap-3 border-b border-[#edf1f2] pb-3"
          >
            <dt class="text-[#78909f]">Idioma</dt>
            <dd class="font-bold text-[#31536b]">{consoleContext.locale}</dd>
          </div>
          <div
            class="flex justify-between gap-3 border-b border-[#edf1f2] pb-3"
          >
            <dt class="text-[#78909f]">Dirección</dt>
            <dd class="font-bold text-[#31536b]">{consoleContext.direction}</dd>
          </div>
          <div class="flex justify-between gap-3">
            <dt class="text-[#78909f]">Perfil de diseño</dt>
      ...
      <Card variant="flat-large">
        ...
        <div class="mt-4 flex flex-wrap gap-2">
          {#each consoleContext.bootstrap.enabledModules as module}
            <Badge variant="neutral" class="h-auto px-2 py-1 text-[0.62rem]"
              >{module}</Badge
            >
          {/each}
        </div>
        <p class="mt-5 text-xs leading-5 text-[#78909f]">
          Los módulos visibles son simulados para diseño. No representan
          autorización backend definitiva.
        </p>
      </Card>
    </div>
  {/if}
