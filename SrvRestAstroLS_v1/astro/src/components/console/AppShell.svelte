...
<div class="min-h-screen bg-[#f3f7f7] text-[#203c55]" dir={consoleContext.direction} data-hydrated={isHydrated ? "true" : "false"}>
    ...
    <div class="mx-auto max-w-[104rem] px-4 py-5 sm:px-6 lg:px-8 lg:py-7">
      ...
      <div class="mt-6 grid gap-6 2xl:grid-cols-[minmax(0,1fr)_17rem]">
        ...
        <aside class="hidden space-y-4 2xl:block">
          <section class="rounded-2xl border border-card-border bg-card-bg p-4">
            <p class="top-badge-neutral">Workspace activo</p>
            <p class="mt-2 text-lg font-bold text-[#31536b]">{consoleContext.activeWorkspace.name}</p>
            <p class="mt-1 text-sm leading-5 text-[#8396a2]">{consoleContext.activeOrganization.name}</p>
            <div class="mt-4 flex flex-wrap gap-2">
              <Badge variant="info" class="h-auto px-2 py-1 text-sm">{consoleContext.activeWorkspace.locale}</Badge>
              <Badge variant="neutral" class="h-auto px-2 py-1 text-sm">{consoleContext.activeWorkspace.direction}</Badge>
            </div>
          </section>
          ...
          <section class="rounded-2xl border border-card-border bg-card-bg p-4">
            ...
            <div class="mt-3 space-y-3">
              {#each pendingTasks.slice(0, 3) as task}
                <article class="border-b border-card-border-mini pb-3 last:border-0 last:pb-0">
                  <p class="text-sm font-bold leading-5 text-[#47657b]">{task.title}</p>
                  <p class="mt-1 text-xs text-[#91a2ad]">Vence {formatDate(task.dueDate, consoleContext.locale)}</p>
                </article>
              {:else}
                <p class="text-sm leading-5 text-[#8396a2]">No hay tareas pendientes.</p>
              {/each}
            </div>
          </section>
          ...
          <section class="rounded-2xl border border-card-border bg-card-bg p-4">
            ...
            <div class="mt-3 space-y-3">
              {#each recentReports as report}
                <article class="border-b border-card-border-mini pb-3 last:border-0 last:pb-0">
                  <p class="text-sm font-bold leading-5 text-[#47657b]">{report.title}</p>
                  <p class="mt-1 text-xs text-[#91a2ad]">{report.period}</p>
                </article>
              {:else}
                <p class="text-sm leading-5 text-[#8396a2]">Aún no hay reportes en este workspace.</p>
              {/each}
            </div>
          </section>
        </aside>
