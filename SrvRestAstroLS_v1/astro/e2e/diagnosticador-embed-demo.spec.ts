...
test.describe("Diagnosticador Embed Demo", () => {
  test.setTimeout(120000);

  const EMBED_SESSION_KEY = "team360.embed.demo.session.v1";
  const VERA_SESSION_KEY = "team360.vera.session.v1";

  test("carga el wrapper, firma server-side y envia turn embed sin contexto tenant en body", async ({ page }) => {
    ...
    await page.goto("/t360-embed-demo");
    ...
    if (!authRequestBody || !authResponseBody || !turnRequestBody) {
      throw new Error("Embed auth or turn request was not captured");
    }

    const authBody: Record<string, unknown> = authRequestBody!;
    const authResponse: Record<string, unknown> = authResponseBody!;
    const turnBody: Record<string, unknown> = turnRequestBody!;
    ...
    expect(embedSession).not.toBeNull();
    const embedData = JSON.parse(embedSession!);
    expect(typeof embedData.session_id).toBe("string");
