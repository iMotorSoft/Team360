...
- Cuando el usuario escriba `contexto componentes`, leer `lat.md/diagnosticador-embeddable-component-architecture.md` antes de analizar o modificar componentes, paquetes, estilos, interaction blocks, adapters, tests E2E o estructura frontend del Diagnosticador embebible.
...
- Para diagramas tecnicos, usar `lat.md/team360-mermaid-diagram-policy.md`: Mermaid es la fuente canonica versionable; SVG/PNG/Excalidraw son derivados opcionales; no hay instalacion global obligatoria ni se adopta el skill gstack `/diagram` completo.
- Para bugs no triviales, especialmente cuando la prueba manual falla aunque los tests pasen, usar `lat.md/team360-root-cause-debugging-policy.md`: reproducir, nombrar hipotesis de causa raiz, confirmar evidencia, aplicar fix minimo y agregar regresion backend/Playwright cuando corresponda.
